...
1. Copy `.env.example` to `.env` and fill the four public values. Never add OpenAI, Clerk secret, or worker keys.
...
- Deploy the worker using the root `railway.toml` and observe `/health` plus job logs for 48 hours.
