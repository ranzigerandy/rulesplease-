...
import { Check, FileText, Languages, Plus, RefreshCw, Search, ShieldCheck, X } from "lucide-react-native";
...
export default function RulebookInfoScreen() {
  ...
  return <Screen><Header title="Rulebook info" coverUrl={thumbnailUrl(row.game.bggId)} />
    ...
    <ExpansionPicker visible={expansionsOpen} baseGameId={row.game.bggId} onClose={() => setExpansionsOpen(false)} />
    <ImportRulebook visible={importOpen} row={row} onClose={() => setImportOpen(false)} />
  </Screen>;
}

function ExpansionPicker({ visible, baseGameId, onClose }: { visible: boolean; baseGameId: number; onClose: () => void }) {
  ...
  async function submit() {
    setAdding(true);
    try {
      await Promise.all(selected.map((expansion) => addGame({ game: { bggId: expansion.id, name: expansion.name, year: expansion.year, rank: expansion.rank, average: expansion.average, usersRated: expansion.users, isExpansion: true, thumbnailUrl: thumbnailUrl(expansion.id) } })));
